        ...
        foreach (var dir in dirs)
        {
            ...
            foreach (var file in dir.GetFiles("*.json", recursive: true))
            {
                try
                {
                    ...
                    var path = Path.Combine(parent, newFileName);
                    GenerationPackIO.Save(bakaV2, path);

                    File.Move(file, dir_json.Combine(parent).EnsureDirectoryExist().Combine(Path.GetFileName(file)));

                    sw.Log($"Migrated {file}");
                }
                catch (Exception e)
                {
                    Print($"MIGRATION FAIL | {file} | {e.GetErrorMessage()}");
                }
            }
        }
